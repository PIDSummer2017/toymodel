...
y_ = tf.placeholder(tf.float32, [None, 4,5],name='labels')
...
with tf.name_scope('cross_entropy'):
  ...
  totalerr = tf.add_n(cross_entropy_total)
...
for i in range(cfg.TRAIN_ITERATIONS):
    ...
    if i%1000 ==0:
        ...
        test_accuracy = accuracy.eval(feed_dict={x:batchtest[0], y_0:batchtest[1][0], y_1:batchtest[1][1], y_2:batchtest[1][2], y\
_3:batchtest[1][3]})
        print("step %d, test accuracy %g"%(i, test_accuracy))

# post training test
batch = make_images(cfg.TEST_BATCH_SIZE,debug=cfg.DEBUG)
print("Final test accuracy %g"%accuracy.eval(feed_dict={x: batch[0], y_0: batch[1][0], y_1:batch[1][1], y_2:batch[1][2], y_3:batc\
h[1][3]}))
