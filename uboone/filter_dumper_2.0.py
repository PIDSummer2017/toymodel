...
#    ax.set_ylim(*ylim)
#    ax.set_xlim(*xlim)
#img.write_png(str(cfg.LOGDIR)+'/Entry%05d.png' % entry)
#plt.close()
...
def plotNNFilter(units):
    ...
    for i in range(filters):
        ...
        img.write_png(str(cfg.LOGDIR)+'/Entry'+str(entry)+'_Filter' +str(i)+'.png')
        plt.close()
...
op_to_restore = graph.get_tensor_by_name("%s" % sys.argv[3])
