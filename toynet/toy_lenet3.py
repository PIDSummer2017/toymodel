...
def build(input_tensor, num_class=4):
    ...
    net = input_tensor
    # 1st conv layer                                                            
    net = L.conv2d(input_tensor=net, name='conv1_1', kernel=(3,3), stride=(1,1), num_filter=256, activation_fn=tf.nn.relu)
    # max pool                                                                 
    net = L.max_pool (input_tensor=net, name="pool1",   kernel=(2,2), stride=(2,2))
    # 2nd conv layer                                                           
    net = L.conv2d(input_tensor=net, name='conv2_1', kernel=(3,3), stride=(1,1), num_filter=256, activation_fn=tf.nn.relu)
    # max pool                                                                 
    ...
    # max pool                                                                 
    net = L.max_pool (input_tensor=net, name="pool3",   kernel=(2,2), stride=(2,2))
